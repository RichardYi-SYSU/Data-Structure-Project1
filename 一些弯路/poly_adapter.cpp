...
// --- 小工具：把 double 变成紧凑字符串（去掉多余的0与小数点）---
...
// fromString：支持形如  -2.5x^3 + 7x - 1 、 x^2-1 、 -x 、 3  等
PolyNode* fromString(const std::string& src) {
    ...
    while (i < n) {
        ...
        if (std::fabs(coef) > EPS) {
            // 交给你的插入函数（会按指数降序插入并合并同幂）
            InsertTerm(head, coef, exp);  // 已在你的实现中处理零系数过滤与同幂合并:contentReference[oaicite:2]{index=2}
        }
...
// toString：把链表变回如 3x^2 - 5x + 1 的人类可读格式
